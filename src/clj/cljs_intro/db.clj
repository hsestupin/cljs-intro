...
(def dbconn {:classname "org.sqlite.JDBC"
             :subprotocol "sqlite"
...
(defn
  ^{:doc "Gets the demographic information for the player"}
  get-player-demog [last-name]
  (run-sql ["select * from master where lastname like ? " last-name]))

(defn
  ^{:doc "Gets goals, assists, shots, etc for any player who has recorded at least
	 one offensive stat in his career"}
  get-scoring-stats [player-id]
  (run-sql ["select * from scoring where playerid = ?" player-id]))

(defn
  ^{:doc "Gets goal against, save percentage, etc for any player who played in the goal"}
  get-goalie-stats [player-id]
  (run-sql ["select * from goalies where playerid = ?" player-id]))

(defn
  ^{:doc "Gets the players demographics, scoring stats and goalie stats returned
